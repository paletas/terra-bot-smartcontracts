...
use cosmwasm_std::{
    to_binary,
    Decimal, Uint128, Addr,
    CosmosMsg, WasmMsg, WasmQuery,
    StdResult,
    Deps, QuerierWrapper, QueryRequest,
    Coin
};
...
fn asset_to_terraswap_asset(asset: &Asset, amount: Option<Uint128>) -> TerraswapAsset {
    match &asset.info {
        AssetInfo::Token { .. } => TerraswapAsset { info: asset_info_to_terraswap_info(&asset.info), amount: amount.unwrap_or(asset.amount) },
        AssetInfo::NativeToken { .. } => TerraswapAsset { info: asset_info_to_terraswap_info(&asset.info), amount: amount.unwrap_or(asset.amount) }
    }
}

fn asset_info_to_terraswap_info(info: &AssetInfo) -> TerraswapAssetInfo {
    match info {
        AssetInfo::Token { contract_addr } => TerraswapAssetInfo::Token { contract_addr: contract_addr.to_string() },
        AssetInfo::NativeToken { denom } => TerraswapAssetInfo::NativeToken { denom: denom.to_string() }
    }
}
...
fn query_pair_info(
    ...
    querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
        ...
        msg: to_binary(&FactoryQueryMsg::Pair {
            asset_infos: [asset_info_to_terraswap_info(&asset_infos[0]), asset_info_to_terraswap_info(&asset_infos[1])],
        })?,
    }))
}

impl LiquidityPoolSwapMsg {
    pub fn create_execution_message(&self, deps: Deps, offer_asset: Asset, ask_asset_info: AssetInfo, to: String) -> StdResult<CosmosMsg<TerraMsgWrapper>> {
        ...
        match offer_asset.info.clone() {
            AssetInfo::NativeToken { denom } => {
                // deduct tax first
                let amount = offer_asset.amount.checked_sub(offer_asset.compute_tax(&deps.querier)?)?;
